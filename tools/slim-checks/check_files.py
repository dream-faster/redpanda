...
ATTRS = ('srcs', 'hdrs', 'src', 'out', 'outs', 'definitions', 'data',
         'textual_hdrs', 'implementation_hdrs')
bad = 0
for build in glob.glob('src/**/BUILD', recursive=True):
    ...
    for attr in ATTRS:
        for m in re.finditer(
          r'\b' + attr + r'\s*=\s*(\[[^\]]*\]|"[^"]*")', s, re.S):
            for name in re.findall(r'"([^"]+)"', m.group(1)):
                if name.startswith(('//', '@', ':')) or '$' in name:
                    continue
                # generated outputs and codegen results never exist on disk
                if name.endswith(('.json.hh', '.pb.h', '.pb.cc', '.proto.h',
                                  '.proto.cc', '_service.h', '.h.inc')):
                    continue
                if attr in ('out', 'outs'):
                    continue
