...
for line in open('src/v/config/configuration.h'):
    ...
    if m and ('property<' in line or 'property;' in line or line.strip().startswith(('property','enum_property','one_or_many','bounded_property','deprecated_property','retention_duration_property','hidden_when_default_property','throughput_control','config::property'))):
        decl.append(m.group(1))
    elif m and re.match(r'^\s+[A-Za-z_][A-Za-z0-9_:<>,\s\*&\.]*\s+[a-z_][a-z0-9_]*;\s*$', line):
        decl.append(m.group(1))
