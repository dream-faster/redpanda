...
TAIL = r'(?:>|\b(?:milliseconds|seconds|size_t|bool|int|int8_t|int16_t|int32_t|'\
       r'int64_t|uint8_t|uint16_t|uint32_t|uint64_t|double|float|sstring|void|auto))'
HEAD = r'(?:virtual\b|static\b|const\b|std::|ss::|absl::|model::|cluster::|config::|'\
       r'kafka::|security::|storage::|chunked_|tristate<|std::optional<)'
RX = re.compile(TAIL + r'\s+' + HEAD)

files = [f for f in subprocess.run(['git', 'ls-files', 'src/v'], capture_output=True,
