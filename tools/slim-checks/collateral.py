...
KEYWORDS = re.compile(
    ...
    r"|sr_|avro|protobuf_schema|subject|compatibility",
    re.I,
)

MIN_RUN = int(sys.argv[1]) if len(sys.argv) > 1 else 6
base = subprocess.run(
    ["git", "rev-parse", "upstream/v26.2.x"], capture_output=True, text=True
).stdout.strip()
diff = subprocess.run(
    ["git", "diff", "-U0", base, "--", "src/v"], capture_output=True
