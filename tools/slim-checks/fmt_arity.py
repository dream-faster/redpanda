    ...
    for call in ('fmt::format_to(', 'vlog('):
        ...
        while True:
            ...
            # find the argument holding the format string
              ...
              (k for k, a in enumerate(args) if '"' in a and '_MSG' not in a),
              None)
            if fi is None:
                continue
            ...
            if not lits:
                continue
            fstr = ''.join(lits)
            holes = len(re.findall(r'(?<!\{)\{[^{}]*\}', fstr.replace('{{', '\x01').replace('}}', '\x02')))
...
files = [f for f in subprocess.run(['git', 'ls-files', 'src/v'], capture_output=True, text=True).stdout.split()
         if f.endswith(('.cc', '.h'))]
total = 0
for f in files:
    try:
